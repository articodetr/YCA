...
import {
  Loader2, CheckCircle, AlertCircle, User, X, Scale, Send, Upload, Crown,
} from 'lucide-react';
...
interface OtherLegalBookingFormProps {
  onComplete: (result: BookingResult) => void;
}

const translationsData = {
  en: {
    title: 'Other Legal / Documentation Request',
    subtitle: 'Submit your legal or documentation request',
    ...
    backToForm: 'Back',
    legalService: 'Legal/Documentation Service',
  },
  ar: {
    title: 'طلب قانوني / توثيق آخر',
    subtitle: 'قدم طلبك القانوني أو التوثيقي',
    ...
    urgencyOptions: {
      low: 'منخفض - لا يوجد موعد نهائي محدد',
      medium: 'متوسط - خلال بضعة أسابيع',
      high: 'مرتفع - خلال أسبوع',
      urgent: 'عاجل - خلال 48 ساعة',
    } as Record<string, string>,
    ...
    backToForm: 'رجوع',
    legalService: 'خدمة قانونية / توثيق',
  },
};

export default function OtherLegalBookingForm({ onComplete }: OtherLegalBookingFormProps) {
  ...
  if (step === 'payment' && clientSecret && formPayload) {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden" dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="bg-gradient-to-r from-amber-600 to-amber-700 px-6 sm:px-8 py-6">
        ...
        <div className="p-6 sm:p-8">
          ...
          {stripePromise ? (
            <Elements stripe={stripePromise} options={elementsOptions}>
              <ServiceCheckoutForm amount={paymentAmount} serviceLabel={t.legalService} formPayload={formPayload} onSuccess={handlePaymentSuccess} onBack={() => setStep('form')} />
            </Elements>
          ) : (
      ...
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border border-gray-200">
          <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-5">
            <div className="p-1.5 sm:p-2 bg-amber-100 rounded-lg flex-shrink-0"><User className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" /></div>
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <input type="text" value={formData.fullName} onChange={e => setFormData(p => ({ ...p, fullName: e.target.value }))}
                className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" required disabled={dataLoading} />
            </div>
            <div>
        ...
        <div className="bg-gray-50 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border border-gray-200">
          <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-5">
            <div className="p-1.5 sm:p-2 bg-amber-100 rounded-lg flex-shrink-0"><Scale className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" /></div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))} rows={5}
                ...
                className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none" required disabled={dataLoading} />
            </div>
            <div>
              ...
              <select value={formData.urgency} onChange={e => setFormData(p => ({ ...p, urgency: e.target.value }))}
                className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent appearance-none" disabled={dataLoading}>
        ...
        <div className="bg-gray-50 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border border-gray-200">
          <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-5">
            <div className="p-1.5 sm:p-2 bg-amber-100 rounded-lg flex-shrink-0"><Upload className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" /></div>
        ...
        <div className="flex items-start gap-2">
          <input type="checkbox" id="legal-consent" checked={consent} onChange={e => setConsent(e.target.checked)}
            className="mt-1 w-4 h-4 text-amber-600 border-gray-300 rounded focus:ring-amber-500" disabled={dataLoading} />
          <label htmlFor="legal-consent" className="text-sm text-gray-700">{t.consentLabel}</label>
        </div>
