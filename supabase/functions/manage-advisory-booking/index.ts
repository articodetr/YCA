...
function buildActionEmail(application: any, action: 'cancel' | 'reschedule', overrides?: Record<string, string>) {
  ...
  const details = [
    ...
    ['Date / التاريخ', overrides?.booking_date ? formatDate(overrides.booking_date) : formatDate(application.booking_date)],
...
async function releaseSlots(adminClient: any, application: any, serviceId: string) {
  if (application.duration_minutes === 60) {
    ...
    const currentIndex = (allSlots || []).findIndex((slot: any) => slot.id === application.slot_id);
    ...
    const releaseOne = await adminClient.rpc('release_availability_slot', { p_slot_id: application.slot_id });
    if (releaseOne.error || !releaseOne.data?.success) {
      throw new Error(releaseOne.error?.message || releaseOne.data?.error || 'Failed to release first slot');
    }

    const releaseTwo = await adminClient.rpc('release_availability_slot', { p_slot_id: nextSlotId });
    if (releaseTwo.error || !releaseTwo.data?.success) {
      throw new Error(releaseTwo.error?.message || releaseTwo.data?.error || 'Failed to release second slot');
    }

    return;
  }

  const release = await adminClient.rpc('release_availability_slot', { p_slot_id: application.slot_id });
  if (release.error || !release.data?.success) {
    throw new Error(release.error?.message || release.data?.error || 'Failed to release slot');
  }
}

async function reserveSlots(adminClient: any, params: {
  serviceId: string;
  bookingDate: string;
  slotId: string;
  ...
  if (params.durationMinutes === 60) {
    ...
    if (allSlotsError) throw allSlotsError;
    const currentIndex = (allSlots || []).findIndex((slot: any) => slot.id === params.slotId);
    if (currentIndex === -1 || currentIndex >= (allSlots || []).length - 1) {
    ...
    if (reserve.error || !reserve.data?.success) {
      throw new Error(reserve.error?.message || reserve.data?.error || 'Failed to reserve slots');
    }

    return;
  }

  const reserve = await adminClient.rpc('reserve_availability_slot', { p_slot_id: params.slotId });
  if (reserve.error || !reserve.data?.success) {
    throw new Error(reserve.error?.message || reserve.data?.error || 'Failed to reserve slot');
  }
}


async function releaseReservedSelection(adminClient: any, params: {
  serviceId: string;
  bookingDate: string;
  slotId: string;
  durationMinutes: 30 | 60;
  ...
  if (params.durationMinutes === 60) {
    ...
    await adminClient.rpc('release_availability_slot', { p_slot_id: params.slotId });
    await adminClient.rpc('release_availability_slot', { p_slot_id: allSlots[currentIndex + 1].id });
    return;
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, applicationId, newSlotId, newBookingDate, newStartTime, newEndTime, newDurationMinutes } = body || {};
    ...
    const { data: application, error: appError } = await adminClient
      ...
      .select('id, user_id, full_name, email, booking_reference, booking_date, start_time, end_time, duration_minutes, slot_id, status, service_type')
      .eq('id', applicationId)
      .single();
    ...
    if (application.user_id !== userData.user.id) {
      return new Response(JSON.stringify({ error: 'You are not allowed to modify this booking' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!String(application.service_type || '').startsWith('advisory_')) {
      return new Response(JSON.stringify({ error: 'This request is not an advisory booking' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!application.booking_date || !application.start_time) {
      return new Response(JSON.stringify({ error: 'Booking date/time is missing' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (hoursUntilBooking(application.booking_date, application.start_time) < 3) {
      return new Response(JSON.stringify({
        error: 'Appointments can only be changed or cancelled at least 3 hours before the booked time.',
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    if (currentSlotError || !currentSlot?.service_id) {
      return new Response(JSON.stringify({ error: 'Current service slot not found' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (action === 'cancel') {
      ...
      try {
        ...
        if (recipient) {
          await sendEmails([{ to: recipient, subject: 'Appointment cancelled / تم إلغاء الموعد', html: buildActionEmail(application, 'cancel') }]);
        }
      } catch (emailError) {
    ...
    if (!newSlotId || !newBookingDate || !newStartTime || !newEndTime || !newDurationMinutes) {
    ...
    if (newSlotError || !newSlot || !newSlot.is_available) {
      return new Response(JSON.stringify({ error: 'The selected new slot is no longer available' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
