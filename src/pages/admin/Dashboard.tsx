...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-2 sm:gap-3 lg:gap-4">
        {statCards.map((card, index) => {
          ...
          return (
            <Link
              key={index}
              to={card.link}
              className="relative bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 border border-slate-200/80 hover:border-slate-300 hover:shadow-md transition-all duration-200 group"
            >
              {card.badge && (
                <span className="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 w-4 h-4 sm:w-5 sm:h-5 bg-rose-500 text-white text-[8px] sm:text-[10px] font-bold rounded-full flex items-center justify-center">
                  {card.badge}
                </span>
              )}
              <div className={`${card.iconBg} w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center mb-2 sm:mb-3`}>
                <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${card.iconColor}`} />
              </div>
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-slate-900 leading-tight">{card.value}</p>
              <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 line-clamp-1">{card.title}</p>
              <p className="text-[9px] sm:text-[11px] text-slate-400 mt-0.5 line-clamp-1 hidden sm:block">{card.subtitle}</p>
              <ArrowUpRight className="w-3 h-3 sm:w-4 sm:h-4 text-slate-300 group-hover:text-emerald-500 absolute top-3 sm:top-4 right-3 sm:right-4 transition-colors" />
            </Link>
          );
        })}
      </div>
      ...
      <div>
        <h2 className="text-xs sm:text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2 sm:mb-3">Quick Actions</h2>
          ...
          {quickActions.map((action) => {
            ...
            return (
              <Link
                key={action.path}
                to={action.path}
                className="flex flex-col items-center gap-1.5 sm:gap-2 py-2 sm:py-3 px-1 sm:px-2 rounded-lg sm:rounded-xl border border-slate-200/80 bg-white hover:border-slate-300 hover:shadow-sm transition-all duration-200 group"
              >
                <div className={`${action.color} w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center`}>
                  <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                </div>
                <span className="text-[10px] sm:text-xs font-medium text-slate-600 group-hover:text-slate-900 transition-colors text-center line-clamp-2">
                  {action.label}
                </span>
              </Link>
            );
